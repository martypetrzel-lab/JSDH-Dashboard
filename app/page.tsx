...
export default async function Home() {
  ...
  const now = new Date();
  const [members, unavailability, recurringUnavailability, template, settings, currentService, shoppingItems] = await Promise.all([
  ...
  const conditioning:ConditioningData={members:members.map(member=>({id:member.id,name:`${member.firstName} ${member.lastName==='—'?'':member.lastName}`.trim(),dt:member.dt,canDrive:member.canDrive,medicalValidUntil:member.medicalValidUntil?member.medicalValidUntil.toISOString().slice(0,10):null,systemAccount:member.systemAccount})),dtActivities:members.flatMap(member=>member.dtActivities.map(serializeDtActivity)),driverActivities:members.flatMap(member=>member.driverActivities.map(serializeDriverActivity)),warningDays:settings?.conditioningWarningDays??30};
