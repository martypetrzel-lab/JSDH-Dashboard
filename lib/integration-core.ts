...
export function effectiveIntegrationCrew(service: IntegrationServiceSource, at: Date) {
  ...
  if (temporary.length === 4 && new Set(temporary.map((item) => item.memberId)).size === 4) return temporary.map((item) => {
    ...
    return { assignmentId: item.originalAssignmentId ?? item.id, memberId: item.memberId, name: integrationMemberName(item.member), role: item.role, slot: item.slot, dt: item.member.dt, baseMemberId: base?.memberId ?? null, baseName: base?.nameSnapshot ?? null, replacingMemberId: base?.memberId === item.memberId ? null : item.memberId, replacingName: base?.memberId === item.memberId ? null : integrationMemberName(item.member), unresolvedReplacement: false };
  });
  return service.assignments.map((assignment) => { const activeReplacements = service.replacements.filter((item) => item.assignmentId === assignment.id && item.from <= at && at < item.to), replacement = activeReplacements.find((item) => item.valid && item.replacementMemberId) ?? null; return { assignmentId: assignment.id, memberId: replacement?.replacementMemberId ?? assignment.memberId, name: replacement?.replacementMember ? integrationMemberName(replacement.replacementMember) : assignment.nameSnapshot, role: assignment.role, slot: assignment.slot, dt: replacement?.replacementMember?.dt ?? assignment.dtSnapshot, baseMemberId: assignment.memberId, baseName: assignment.nameSnapshot, replacingMemberId: replacement?.replacementMemberId ?? null, replacingName: replacement?.replacementMember ? integrationMemberName(replacement.replacementMember) : null, unresolvedReplacement: activeReplacements.some((item) => !item.valid) }; });
}
