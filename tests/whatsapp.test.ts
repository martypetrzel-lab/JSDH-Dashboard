...
void test('WhatsApp popíše dočasný přesun hasiče na velitele',()=>{const message=buildWhatsAppMessage({from:'od',to:'do',commander:'Původní',driver:'Strojník',firefighters:['Bradáč','Hasič'],dt:['Bradáč'],temporaryCrews:[{from:'23.09. 06:00',to:'24.09. 06:00',changes:['Původní – mimo službu','Bradáč: Hasič → Velitel','Žižka: záskok Hasič']}]});assert.match(message,/Bradáč: Hasič → Velitel/);assert.match(message,/Žižka: záskok Hasič/);assert.doesNotMatch(message,/Původní → Bradáč/);});
